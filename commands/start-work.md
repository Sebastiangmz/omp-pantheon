---
description: Start a Sisyphus work session — find Prometheus plan, execute via Atlas
---

<command-instruction>
You are starting a Sisyphus work session.

Spawn the `atlas` agent to execute the work plan via:
```
task(agent: "atlas", tasks: [{ id: "start-work", description: "Execute work plan", assignment: "<full instructions below>" }])
```
...
- `/start-work [plan-name] [--worktree <path>]`
  ...
  - `--worktree <path>` (optional): absolute path to an existing git worktree to work in
    - If specified and valid: pre-set `worktree_path` in `.sisyphus/boulder.json`
