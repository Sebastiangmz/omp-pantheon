...
5. Appends nine `.pi/`-related patterns to `.gitignore` (idempotent, exact-line match).
6. Appends one JSONL line to `.pi/.bootstrap-log.jsonl` (mode `0600`, approver `luci`).

Re-running `--i-approve` on an already-bootstrapped project mutates only the audit log.
