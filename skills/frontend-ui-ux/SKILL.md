...
| Writing or modifying frontend code, OR auditing performance / SEO / accessibility / quality | ALSO `references/perfection/README.md`. Lighthouse 100 in every category, measured on real browser automation (use the `browser` tool or Playwright via `bash`; never the `lighthouse` CLI), achieved through architecture — never by dropping animations or hiding content. |
...
| `react-perf-tooling.md` | Before ANY React audit. The browser-based + `playwright-lighthouse` + `react-scan/lite` injection recipe, per-route render budgets, and the React-specific root-cause checklist. Lighthouse 100 with 30+ unnecessary renders is NOT done. |

Audit approach (build for production first; never measure a dev server):

Use the `browser` tool or Playwright via `bash` to run Lighthouse audits against the production build. Run mobile AND desktop presets, 3–5 runs, take the median, diagnose from the JSON report.
...
- **Verify in a real browser before declaring done.** Screenshots at 375 / 768 / 1280px; hover, focus, loading, empty, and error states all exercised. Use the `browser` tool for visual verification.
...
| Driving a browser for the Design QA phase | Use the `browser` tool directly |
...
**Note:** The upstream skill ships `references/` directories with design, perfection, and ui-ux-db sub-libraries containing 81+ reference files, scripts, and CSV databases. These reference files are not inlined here — load them from the skill directory at runtime when routing indicates they are needed.
